<?php
    function toBarcode($code, $len = 13)
    {
        $upce = false;
        if ($len == 6) {
            $len = 12; // UPC-A
        ...
        return convertToBarcode($bararray);
    }

    function convertToBarcode($oldBarcodeArray) {
        $newBarcodeArray = [];
    ...
    function generateBarcode($code, $widthFactor = 2, $totalHeight = 30, $color = 'black') {

        $barcodeData = toBarcode($code);
